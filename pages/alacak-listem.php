...
<body class="bg-gray-50">
  <div class="content">
    <div class="form-container p-8 w-full max-w-4xl mx-auto">
      <h2 class="text-2xl font-bold text-gray-800 mb-6">Alacak Listesi</h2>
      <div class="mb-6">
      ...
      <div id="alacak-list" class="space-y-4"></div>
    </div>
  </div>
  ...
  <script>
    ...
    function renderPersonList(filteredPeople) {
      ...
      ` : filteredPeople.map(p => `
        <div class="person-item p-2 border-b" onclick="showAlacakForm(${p.ID}, '${p.NAME}')">
    ...
    function loadAlacak() {
      console.log('Loading alacak...');
      fetch('api/alacak.php?action=get_alacak')
        .then(response => {
          if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
          return response.json();
        })
        .then(data => {
          if (data.success) {
            alacakList = data.alacak;
            console.log('Alacak loaded:', alacakList);
            renderAlacakList();
          } else {
            Swal.fire('Hata!', data.message || 'Alacaklar yüklenirken hata oluştu.', 'error');
          }
        })
        .catch(error => {
          console.error('Error loading alacak:', error);
          Swal.fire('Hata!', 'Alacaklar yüklenirken hata oluştu: ' + error.message, 'error');
        });
    }

    function renderAlacakList() {
      console.log('Rendering alacak list...');
      const alacakListDiv = document.getElementById('alacak-list');
      alacakListDiv.innerHTML = alacakList.length === 0 ? `
        <div class="text-center text-gray-500 py-4">Alacak bulunamadı</div>
      ` : `
        <table class="transaction-table">
          ...
          <tbody>
            ${alacakList.map(a => `
              <tr>
                <td>${a.NAME}</td>
                <td class="amount-field">${formatNumber(a.ÜCRET)}</td>
                <td>${a.ACIKLAMA}</td>
                <td>${formatDate(a.TARIH)}</td>
                <td>
                  <button onclick="makePayment(${a.ID}, ${a.ÜCRET}, ${a.KISI_ID}, '${a.NAME}', 'alacak')" 
    ...
    function showAlacakForm(personId, personName) {
      Swal.fire({
        title: `${personName} için Alacak Ekle`,
        html: `
          <div class="text-left">
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            title: 'Emin misiniz?',
            text: `${personName} için ${formatNumber(amount)} ₺ alacak kaydetmek istiyor musunuz?`,
            icon: 'question',
            showCancelButton: true,
            ...
            if (confirmResult.isConfirmed) {
              fetch('api/alacak.php?action=add_alacak', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  KISI_ID: personId,
              ...
              .then(data => {
                if (data.success) {
                  Swal.fire('Başarılı!', 'Alacak kaydedildi.', 'success').then(() => {
                    loadAlacak();
                  });
                } else {
                  console.error('Add alacak failed:', data.message);
                  Swal.fire('Hata!', data.message || 'Alacak kaydedilirken hata oluştu.', 'error');
                }
              })
              .catch(error => {
                console.error('Error saving alacak:', error);
                Swal.fire('Hata!', 'Alacak kaydedilirken hata oluştu: ' + error.message, 'error');
              });
            }
          });
        }
      ...
      Swal.fire({
        ...
        preConfirm: () => {
          ...
          if (paymentAmount > originalAmount) {
            Swal.showValidationMessage('Ödeme tutarı, alacak tutarından fazla olamaz.');
            return false;
          }
          if (!description) {
            Swal.showValidationMessage('Açıklama zorunludur.');
